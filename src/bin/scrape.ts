...
const UNICODE_DATA_URL = 'https://www.unicode.org/Public/13.0.0/ucd/UnicodeData.txt'
...
const EMOJI_DATA_URL = 'https://www.unicode.org/emoji/charts-13.0/emoji-list.html'
const EMOJI_DATA_SEARCH = /<tr><td class='rchars'>\d+<\/td>\n?<td class='code'><a.*?>(.*?)<\/a><\/td>\n?<td class='andr'><a.*?><img alt='(.*?)'.*?<\/td>\n?<td class='name'>(.*?)<\/td>\n?<td class='name'>(.*?)<\/td>/gi

// [1]=hex; [2]=character; [3]=name
const EMOJI_TONE_DATA_URL = 'https://www.unicode.org/emoji/charts-13.0/full-emoji-modifiers.html'
...
const scrape = async () => {
  ...
  fs.writeFileSync('public/unicode/13.0.0.json', JSON.stringify(glyphs), { flag: 'w' })
}
